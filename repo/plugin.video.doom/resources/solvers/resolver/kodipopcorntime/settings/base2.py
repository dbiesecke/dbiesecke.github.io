#!/usr/bin/python
import xbmc, sys, os, time, stat, shutil,xbmcaddon
...
class _MetaClass2(_MetaClass):
    ...
    def _binary_path(cls):
        ...
        binary_path = os.path.join(xbmc.translatePath(__addon__.getAddonInfo('profile')), 'resources', 'bin', "%s_%s" %(Platform.system, Platform.arch), binary).encode(Addon.fsencoding)
        log('binary_path', LOGLEVEL.WARNING)
        log(binary_path, LOGLEVEL.WARNING)
