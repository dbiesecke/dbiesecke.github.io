...
import re,sys,cookielib,urllib,urllib2,urlparse,gzip,StringIO,HTMLParser,time,random,base64,xbmc,logging

from resources.lib.modules import cache
from resources.lib.modules import workers
from resources.lib.modules import dom_parser
from resources.lib.modules import log_utils
from resources.lib.modules import utils,cfscrape
...
def request(url, close=True, redirect=True, error=False, proxy=None, post=None, headers=None, mobile=False, XHR=False, limit=None, referer=None, cookie=None, compression=True, output='', timeout='30'):
        ...
        try:
            ...
            if response.code == 503:
                logging.warning('CF CODE in client')
                scraper = cfscrape.create_scraper()
                response=scraper.get(url,headers=_headers,data=post).content
                
            else:
                log_utils.log('Request-Error (%s): %s' % (str(response.code), url), log_utils.LOGDEBUG)
                if error == False: return  'Request-Error (%s): %s' % (str(response.code), url)
