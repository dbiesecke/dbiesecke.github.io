...
class System(object):
	...
	def restart(self, sleep = True):
		...
		System.execute('RunAddon(%s)' % 'plugin.video.doom')
	...
	@classmethod
	def info(self, value):
		return xbmcaddon.Addon('plugin.video.doom').getAddonInfo(value)
	...
	def window(self, action = None, parameters = {}, command = None, sleep = None, refresh = False):
		if command == None:
			...
			if not parameters == None and not parameters == '' and not parameters == {}:
				...
				parameters = ''
			command = '%s%s/%s' % (System.PluginPrefix, 'plugin.video.doom', parameters)
...
SettingsAddon = xbmcaddon.Addon('plugin.video.doom')
...
class Media(object):
	...
	NameSeasonLong = 'season'#xbmcaddon.Addon('plugin.video.doom').getLocalizedString(32055).encode('utf-8')
	NameSeasonShort = NameSeasonLong[0].upper()
	NameEpisodeLong = 'S'#xbmcaddon.Addon('plugin.video.doom').getLocalizedString(33028).encode('utf-8')
