...
wrapper class for jen list functions
    ...
    def process_item(self, item_xml):
        ...
        mode_in=''
        if len(item["externallink"])>0:
            
            mode_in='dir'
            
            
            
        if item.item_string.startswith("<item>") and mode_in=='':
            title = item["title"]
            if title == "":
                title = item["name"]
            ...
            mode = "get_sources"
            link = item_xml
        elif item.item_string.startswith("<dir>") or mode_in=='dir':
            logging.warning('IN DIR')
            title = item["name"]
            if title == "":
                title = item["title"]
            try:
...
def display_list(items, content_type, pins=False):
    "display jen list in kodi"
    logging.warning('DISPLAY LIST')
    logging.warning(pins)        
    if ADDON.getSetting("enable_cache") == "true" and pins:
