...
#############Defined Strings#############
icon         = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.buchclub1/icon.png'))
fanart       = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.buchclub1/fanart.jpg'))
...
def buildcleanurl(url):
	...
	return url
def start():
    auth = '%s:%s/get.php?username=%s&password=%s&type=m3u'%(user.host,user.port,username,password)
    request = requests.get(auth)
    if not request.status_code == 200:
        yesnowindow = xbmcgui.Dialog().yesno('buchclub 1', 'buchclub 1 Aktivierung: ','','Willst du buchclub 1 aktivieren?', yeslabel='Ja',nolabel='Nein')
        ...
        if yesnowindow == 1:
            ...
            auth = '%s:%s/get.php?username=%s&password=%s&type=m3u'%(user.host,user.port,usern,passw)
            request = requests.get(auth) 
            if not request.status_code == 200: 
	        ...
	        dialog.ok('buchclub 1','Aktivierung abgebrochen.','')
...
def home():
	tools.addDir('Pro TV','live',1,icon,fanart,'')
	tools.addDir('TV Guide','guide',7,icon,fanart,'')
	tools.addDir('Free TV','freetv',18,icon,fanart,'')
	tools.addDir('VOD','vod',3,icon,fanart,'')
	tools.addDir('Suche','search',5,icon,fanart,'')
	tools.addDir('Einstellungen','url',8,icon,fanart,'')
...
def freetv():
	xbmc.executebuiltin('RunAddon(plugin.video.buchclub)')
...
def search():
	...
	if not text:
		...
		return
	xbmc.log(str(text))
	open = tools.OPEN_URL(m3u)
	all_chans = tools.regex_get_all(open,'INF:','EXT')
	for a in all_chans:
		name = tools.regex_from_to(a,'tvg-name="','"')
		url  = tools.regex_from_to(a,'http://li','\r').replace('ve.','http://live.')
		url  = url.replace('%0D%0A','')
		thumb= tools.regex_from_to(a,'tvg-logo="','"')
...
def correctPVR():
    ...
    PVRVOR = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.buchclub1/settingsvor.xml'))
    ...
    M3UNEW = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.buchclub1/favnew.m3u'))
