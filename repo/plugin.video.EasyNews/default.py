...
art = 'http://xunitytalk-repo.googlecode.com/svn/addons/plugin.video.EasyNews/EasyArt/'
#art = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.EasyNews/EasyArt', ''))
...
def CATEGORIES():
        ...
        if ADDON.getSetting('downloads') == 'true':
            addDir('Downloads','url',102,iconimg+'Main/favorites.png','','','','')
...
def MOVIE_CATEGORIES():
        addDir('Search','url',18,art+'searchmovies.png','','','','')
        addDir('Most Popular','http://akas.imdb.com/search/title?num_votes=5000,&sort=moviemeter&title_type=feature',32,art+'newreleases.png','','','','')
        addDir('IMDb US Box Office','http://akas.imdb.com/search/title?num_votes=5000,&sort=boxoffice_gross_us&title_type=feature',32,art+'topmovies.png','','','','')
        addDir('IMDb US Release Date','http://akas.imdb.com/search/title?num_votes=5000,&sort=release_date_us,desc&title_type=feature',32,art+'topmovies.png','','','','')
        addDir('IMDb All Time Top Movies','http://akas.imdb.com/search/title?num_votes=5000,&sort=user_rating,desc&title_type=feature',32,art+'topmovies.png','','','','')
        addDir('IMDb Genres','http://akas.imdb.com/genre',33,art+'genres.png','','','','')
        #addDir('Top LoveFilm Movies','http://www.lovefilm.com/browse/film/dvd/films/?r=50',14,art+'topmovies.png','','','','')
...
def actorsearch():
        ...
        url='http://m.imdb.com'+match[0]+'filmotype/actor'
...
def TMDB(url):
        link=OPEN_URL(url)
        match = re.compile('src="(.+?)" width=".+?" />\n              <div class=".+?">\n                S.+?\n.+?/div>\n.+?/span>\n.+?/a>\n\n.+?div class="info">\n.+?h4><a href=".+?">(.+?)<').findall(link)
...
def TV_SEARCH(name,iconimage):
        ...
        for p in LINK:
            try:
                ...
                name = re.compile('v_li_tt"\n>(.+?)<').findall(p)[0]
                match.append([url,name, iconimage])                          
            except:pass  
        for url, name, iconimage in match:
            ...
            try:
                ...
                pass
            addDir(name,url,10,iconimage,fanart,series,'','')   
        setView('movies', 'tvshows-view')
...
def DOWNLOADS(url):
     ...
     for infile in glob.glob(os.path.join(path, '*.*')):
         ...
         if '.' in name:
             addLink1(name.split('.')[-2],url,iconimg+'Main/favorites.png')
             xbmcplugin.addSortMethod(int(sys.argv[1]), xbmcplugin.SORT_METHOD_VIDEO_TITLE) 
     setView('movies', 'movies-view')  
